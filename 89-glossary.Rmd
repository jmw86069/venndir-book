...
::: {#hidden-overlaps-glossary}
:::
hidden overlaps
  ~ A [Venn overlap](#venn-overlap) which cannot be represented in a Venn
  or proportional Euler diagram, usually due to geometric constraints.
  For three or more sets, proportional Euler diagrams are not guaranteed
  to have a geometric solution when using circle or ellipse shapes.
  
  In Venndir, hidden overlaps are indicated by placing a footnote symbol
  $`r "\u2020"`$, also called a 'dagger', in the bottom-right corner
  of the figure.
...
signed incidence matrix  
  ~ An enhanced [incidence matrix](#g-incidence-matrix) that represents
