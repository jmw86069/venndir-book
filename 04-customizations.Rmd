...
Venndir uses **grid** and is compatible with patchwork.

There are two key steps:

1. Call `venndir()` with arguments:
   * `do_plot=TRUE` which tells Venndir to create the graphical objects
   which will be drawn. By default it is already `TRUE`.
   * `do_draw=FALSE` which tells Venndir not to draw these objects.
   This argument is optional, to prevent drawing the figure.
...
Figure \@ref(fig:venndir-patchwork-1) shows two Venn diagrams side-by-side.

(ref:venndir-patchwork-1) Two Venndir figures combined using the **patchwork** R package.

```{r venndir-patchwork-1, out.width="100%", fig.width=9, fig.height=4.5, fig.cap="(ref:venndir-patchwork-1)", fig.alt="(ref:venndir-patchwork-1)"}
library(patchwork)
...
* The Venndir font sizes are absolute points, and should be adjusted
accordingly.
* The Venndir circles will adjust to the available output size,
while the fonts will remain the same size.
* It works well to decide a target canvas size for the combined figure
output, usually in actual inches or centimeters, then define an
appropriate font size in that context.
* For more details on the Venndir **grid** data, see
...
# clean "-" bc interpreted as a bullet in markdown.
curate_df$sign <- gsub("^-$", "", curate_df$sign);

kdf <- knitr::kable(curate_df,
   booktabs=TRUE,
...
[Wikipedia Unicode](https://en.wikipedia.org/wiki/List_of_Unicode_characters).  
Formats described as `'U+2191'` should be used in R in this form:
`"\u2191"` in order to produce: $`r "\u2191"`$
* **Up arrow** is Unicode U+2191: $`r "\u2191"`$
* **Down arrow** is Unicode U+2193: $`r "\u2193"`$
* **Checkmark** is Unicode U+2713: $`r "\u2713"`$
* **Ballot X** is Unicode U+2717: $`r "\u2717"`$

::::

The following steps were used to create Figure \@ref(fig:curate-check-1):

1. Retrieve the default `curate_df`.
2. Edit the 'agreement' row, and update column 'sign'.
3. Edit the 'mixed' row, and update columns 'sign', and 'color'.
4. Call `venndir()` with arguments `overlap_type='agreement'` and
`curate_df=curate_df`.
...
are typically `-1` and `1` to indicate directionality,
*however any sign values may be used*.

Any sign values not recognized by `curate_df`, as described in
[Customize the Symbols], remain unchanged and un-colored.

Figure \@ref(fig:advanced-signs-1) shows the effect of changing
the signed list values from '1' and '-1', to strings: 'up' and 'dn'.
Because 'up' and 'dn' are not defined in `curate_df`, they remain
unchanged in the output Venndir.

(ref:advanced-signs-1) Venndir using custom values 'up' and 'dn' instead of 1 and -1.

```{r advanced-signs-1, fig.height=5, fig.width=6, fig.cap="(ref:advanced-signs-1)", fig.alt="(ref:advanced-signs-1)"}
...
sign values 'up' and 'dn'.

Further, the sign values 'up' and 'dn' can be assigned to Unicode character
as shown in  [Customize the Symbols]. The value can be a string, or even
[markdown](#g-markdown), described in [Venndir Markdown Support].
The custom value is also shown in the legend.

Figure \@ref(fig:advanced-signs-1f) demonstrates how to customize `curate_df`,
...
(ref:advanced-signs-1f) Venndir showing the custom signs 'up' and 'down' used to produce Unicode signs as before.

```{r advanced-signs-1prep, echo=FALSE, dependson="advanced-signs-1"}
curate_df <- get_venndir_curate_df()
...
```{r advanced-signs-1f, fig.height=5, fig.width=6, echo=FALSE, fig.cap="(ref:advanced-signs-1f)", fig.alt="(ref:advanced-signs-1f)", dependson="advanced-signs-1prep", cache=FALSE}
#
v <- venndir(setlist2,
   fontfamily="Arial",
   curate_df=curate_df,
...
Although directional sign is most often 'up' and 'down',
Venndir can recognize more than two signs.
The following example demonstrates three signs instead of two.
In the simplest form, it uses three letters:
**'a'**, **'b'**, and **'c'**.


(ref:advanced-signs-2) Venndir using three signs: 'a', 'b', and 'c' (left), which are converted to Unicode symbols (right).

```{r advanced-signs-2-show, eval=FALSE}
...
```{r advanced-signs-2, echo=FALSE, fig.ncol=2, out.width="50%", fig.height=5, fig.width=6, fig.cap="(ref:advanced-signs-2)", fig.alt="(ref:advanced-signs-2)", fig.subcap=c("Three signs.", "Three signs using Unicode.")}
setlist <- make_venn_test(n_items=150, n_sets=3, do_signed=TRUE,
   set_names=c("Set 1", "Set 2", "Set 3"))
set.seed(123)
setlist2 <- lapply(setlist, function(i){
...
```{r advanced-signs-2c, eval=FALSE}
curate_df <- get_venndir_curate_df()
# define a, b, c
curate_df[1, "from"] <- "a"
curate_df[2, "from"] <- "b"
...
Figure \@ref(fig:advanced-signs-2) shows the three-sign example
using letters (left), and Unicode symbols (right).
...
Custom signed colors can be defined by editing `curate_df` as described
in [Custom Signs] to this point.

(ref:bw-signed-venn-1) Signed Venn diagrams with sign count labels in black (left), and custom colors.

```{r bw-signed-venn-1, fig.ncol=2, fig.height=6, fig.width=6, out.width="50%", fig.cap="(ref:bw-signed-venn-1)", fig.alt="(ref:bw-signed-venn-1)", fig.subcap=c("Signed labels in black.", "Signed labels in custom colors.")}
# black text labels
curate_df <- get_venndir_curate_df()
...
`make_color_contrast()`, and optional argument `L_lo=65` is passed
...
```{r grobs-2, fig.height=5, fig.width=6, fig.cap="(ref:grobs-2)", fig.alt="(ref:grobs-2)", dependson="grobs-1"}
gtree <- attr(v, "gtree");
grid::grid.newpage();
...
```{r grobs-3, fig.height=5, fig.width=6, fig.cap="(ref:grobs-3)", fig.alt="(ref:grobs-3)", dependson="grobs-1"}
...
The entries are named to be reognizable, for example the grob names
in the `'jps'` entry are listed with `grid::grid.ls()`:

```{r groblist-list, dependson="grobs-3"}
grid::grid.ls(grob_list$jps)
```

Each polygon is named with substring up to the first ':' colon,
after which are optional modifiers indicating the border applied,
in order:

* Each polygon is drawn with outer border (if outerborder is defined),
* then polygon fill and border (if border is defined),
* then inner fill (if innerborder is defined),
* then inner border (if innerborder is defined).

As described in [Venndir Rendering Steps], a polygon is drawn
for each full set, denoted with polygon names having '|set' at the end.
Then each overlap region is drawn, where polygons are named by
the overlap sets.

The example below is somewhat advanced, and demonstrates how to
extract a specific polygon grob, use it to create a custom pattern
fill, then reinsert the new grob into the `grob_list`.

First, extract one overlap region, and draw it to confirm:

(ref:groblist-list-fill) One overlap polygon is extracted from `grob_list` and drawn.

```{r groblist-list-fill, fig.height=5, fig.width=6, fig.cap="(ref:groblist-list-fill)", fig.alt="(ref:groblist-list-fill)", dependson="grobs-3"}
grobname <- "set_C:pathGrob:inner"
use_grob <- grid::getGrob(grob_list$jps, gPath=grobname)
grid::grid.newpage();
grid::grid.draw(use_grob)
```

The next step is to define a grid pattern, in this case it requires
the **gridpattern** R package, loading an image.

Another alternative is to define a color gradient, for example
with `gridpattern::grid.pattern_gradient()`.

```{r groblist-list-pattern}
bg_file <- "images/water_bg.png"
bg_pat <- gridpattern::grid.pattern_image(filename=bg_file,
   type="tile", scale=1, draw=FALSE)
```

Now the grob `use_grob` is used to apply a clipping region to
the pattern fill, so the pattern is only applied to this
specific polygon region.

```{r groblist-clip, dependson=c("groblist-list-pattern", "groblist-list-fill")}
# clip background to the grob
clipped_name <- paste0(grobname, ".pattern")
clipped <- list(gridpattern::clippingPathGrob(
   clippee=bg_pat,
   clipper=use_grob,
   name=clipped_name));
names(clipped) <- clipped_name
```

Now draw the clipped background image:

```{r groblist-clip-draw, fig.height=5, fig.width=6, fig.cap="(ref:groblist-clip-draw)", fig.alt="(ref:groblist-clip-draw)", dependson="groblist-clip"}
grid::grid.newpage();
grid::grid.draw(clipped[[1]])
```

Finally, re-insert the background into the `grob_list`:

```{r insert-grob, dependson="groblist-clip"}
grob_list_jps_list <- grob_list$jps$children;
grobmatch <- match(grobname, names(grob_list_jps_list));

if (grobmatch == 1) {
   grob_list_jps_list <- append(grob_list_jps_list,
      clipped,
      after=grobmatch - 1)
} else {
   grob_list_jps_list <- append(clipped,
      grob_list_jps_list,
      after=1)
}
# insert into grob_list
grob_list$jps <- grid::grobTree(
   children=do.call(grid::gList, grob_list_jps_list),
   name="jps")
```

Bonus points: Re-create the grob tree `gTree` object:

```{r make-gtree, dependson="insert-grob"}
# make new gTree
jps_gtree <- grid::grobTree(
   children=do.call(grid::gList, grob_list),
   name="venndir.figure")
```

Drawing the `gTree` is sometimes more convenient than drawing
each element in the `grob_list`:

(ref:draw-new-gtree) The custom Venndir figure is drawn, using pattern fill in one overlap region.

```{r draw-new-gtree, fig.height=5*1.5, fig.width=6*1.5, fig.cap="(ref:draw-new-gtree)", fig.alt="(ref:draw-new-gtree)", dependson="make-gtree"}
grid::grid.newpage()
grid::grid.draw(jps_gtree)
```

Note that the original polygon fill is drawn on top of the pattern
fill, but this layer could be removed so the pattern is shown
without interference. This change is left to the reader.
